...
```{r PlayerExample, eval = TRUE, echo=T, message=FALSE, tidy=TRUE, results='markup'}
####Tiger####
...
```
`r round(mean(mean(summaryStat[,c(2,4,6)], na.rm=T))*100, 1)`%
```
Average Percentage of Total Winnings from **6** Tournaments in a Year
```
`r round(mean(mean(summaryStat[,c(3,5,7)], na.rm=T))*100, 1)`%
```